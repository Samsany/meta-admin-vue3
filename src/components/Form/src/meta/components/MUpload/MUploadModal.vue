<template>
  <BasicModal @register="registerModal" :title="modalTitle" :width="width" @ok="handleOk" v-bind="$attrs">
    <MUpload ref="uploadRef" :value="value" v-bind="uploadBinds.props" @change="emitValue" />
  </BasicModal>
</template>

<script lang="ts" setup name="MUploadModal">
...
const [registerModal, { closeModal }] = useModalInner(async data => {
  ...
  await nextTick(() => uploadRef.value.addActionsListener())
})
