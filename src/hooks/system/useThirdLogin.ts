...
import { setThirdCaptcha } from '/@/api/system/user'
...
export function useThirdLogin() {
  ...
  //加载中
  const thirdCreateUserLoading = ref(false)
  //绑定手机号
  ...
  function onThirdLogin(source) {
    const url = `http://localhost:20001/oauth/login/${source}`
    window.open(
      url,
      `login ${source}`,
  ...
  function thirdLoginUserCreate() {
    thirdCreateUserLoading.value = true
    // 账号名后面添加两位随机数
    ...
    defHttp
      ...
      .finally(() => {
        thirdCreateUserLoading.value = false
      })
  }
  // 核实密码
